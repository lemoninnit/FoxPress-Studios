...
const liquidbuttonVariants = cva(
  "inline-flex items-center transition-colors justify-center cursor-pointer gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-[color,box-shadow] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        default: "bg-transparent hover:scale-105 duration-500 transition text-inherit",
        destructive:
...
function LiquidButton({
  ...
  return (
    <Comp
      data-slot="button"
      className={cn(
        "relative group overflow-hidden transition-all duration-500 [transition-timing-function:cubic-bezier(0.16,1,0.3,1)] border border-white/10 shadow-[inset_0_1.5px_1.5px_rgba(255,255,255,0.2),0_4px_15px_rgba(0,0,0,0.3)] backdrop-blur-md",
        liquidbuttonVariants({ variant, size, className })
      )}
      {...props}
    >
      {/* Light sheen overlay */}
      <div className="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/5 to-white/10 opacity-100 transition-opacity duration-300 pointer-events-none rounded-[inherit]" />
      
      {/* Gloss border reflection highlight */}
      <div className="absolute inset-0 rounded-[inherit] border border-white/10 pointer-events-none group-hover:border-white/20 transition-colors duration-300" />
